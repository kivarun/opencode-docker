...
- Worker `run`/`pull` go through the docker-helper **HTTP API over its unix socket** (bearer header + `environment` map); env values must never appear in process argv. The CLI is used only for `session create/delete`, with a minimal filtered env (`HOME`, `XDG_CONFIG_HOME`, `XDG_RUNTIME_DIR`, `DOCKER_HELPER_CONFIG`).

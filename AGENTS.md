...
- All docker-helper interaction goes through the **official CLI 2.1.0** (`session create/delete`, `pull`, `run`), spawned as an argv array (`Bun.spawn`, no shell). `session create/delete` run with a minimal filtered env (`HOME`, `XDG_CONFIG_HOME`, `XDG_RUNTIME_DIR`, `DOCKER_HELPER_CONFIG`); `pull`/`run` run with the child Session bearer only (`DOCKER_HELPER_SESSION_TOKEN`), which is repeated as `--env` so the worker receives its Session capability. Worker env values are passed as separate `--env KEY=VALUE` arguments; until docker-helper issue #3 is implemented, resolved worker env (including secrets and the OpenCode config content) is visible in the `docker-helper run` process argv — a consciously accepted temporary risk. They never appear in state files or the orchestrator's own diagnostics. The narrow `GET /auth` socket check remains for Launcher authority validation (no equivalent CLI command). Worker output is inherited from the CLI process (a single mixed stdout/stderr stream) and is treated as untrusted (never a safe audit log or authoritative state); the orchestrator implements no polling, log cursor, or operation-API parsing.

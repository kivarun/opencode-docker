...
- Pipelines: `pipelines/default/` is the declarative pipeline bundle (schema version 1: `pipeline.yaml` + `prompts/` + `schemas/`); it is copied to `/opt/orchestrator/pipelines/` by `Dockerfile_orchestrator` (`# ci: watch=pipelines/**`). `orchestrator/src/pipeline.ts` implements `parsePipelineSpec`/`loadPipeline` (loader + fail-closed validator only — the graph is NOT executed, `agent-smoke` does not use it, and there is no pipeline CLI command yet). Only `pipeline.yaml` is supported (never `.yml`/JSON); states/inputs/transitions are lists; cycle freedom is by design (the run is bounded by `max_transitions`). The bundle root must be a directory and `pipeline.yaml` itself obeys the same canonical bundle containment as the other bundle files; workspace input paths must be unique (case-sensitive, lexical check only); the resolved pipeline carries the accepted `schema_version`.

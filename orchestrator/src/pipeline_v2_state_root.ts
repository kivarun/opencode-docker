...
function requireCleanAbsolutePath(
  value: string,
  variable: string,
): string {
  if (
    ...
    // The offending environment value is never echoed back: the variable
    // name and the expected shape are enough for the operator.
    throw new Error(
      `${variable} must be set to a non-empty absolute clean path (no trailing or doubled slashes, no "." components)`,
    );
  }
  return value;
}
 ...
 * - a variable that is present must be non-empty, absolute and clean; an
 *   impossible local root is a CLI configuration error.
 */
export function resolvePipelineV2StateRootProjection(
  ...
  if (isSet(explicitLocal)) {
    localRoot = requireCleanAbsolutePath(explicitLocal!, "ORCHESTRATOR_STATE_ROOT");
  } else {
    const xdgStateHome = env.XDG_STATE_HOME;
    if (isSet(xdgStateHome)) {
      localRoot = requireCleanAbsolutePath(xdgStateHome!, "XDG_STATE_HOME").replace(/\/+$/, "") + "/orchestrator";
    } else {
      const home = env.HOME;
      if (!isSet(home)) {
        throw new Error(
      ...
      localRoot = requireCleanAbsolutePath(home!, "HOME").replace(/\/+$/, "") + "/.local/state/orchestrator";
    }
  }
  const explicitDaemon = env.ORCHESTRATOR_DAEMON_STATE_ROOT;
  const daemonRoot = isSet(explicitDaemon)
    ? requireCleanAbsolutePath(explicitDaemon!, "ORCHESTRATOR_DAEMON_STATE_ROOT")
    : localRoot;
